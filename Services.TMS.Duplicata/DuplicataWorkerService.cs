...
namespace Services.TMS.Duplicata
{
    public class DuplicataWorkerService : WorkerService
    {
        ...
        protected override TimeSpan IntervaloDeExecução => TimeSpan.FromMilliseconds(int.Parse(_configuration["Configs:Workers:Duplicata:IntervaloEmMilissegundos"]));

        protected override async Task<int> Processar()
        {
            try
            {
                var listaDuplicata = await ApiClientFactory.Instance.GetDuplicataPendente();
                foreach (var duplicata in listaDuplicata.faturas)
                {
                    var duplCompleta = await ApiClientFactory.Instance.PostDuplicataCompleta(duplicata);
                    ...
                    {
                        var retornoDupl = new DuplicataRetornoModel();
                        var retorno = await ApiClientFactory.Instance.PostDuplicataRetorno(retornoDupl);
                    }
